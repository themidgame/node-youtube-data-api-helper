...
function Paginator(options){
    ...
    if(!component._options.endpoint || !component._options.params ){
      ...
      return component.getPage().then(function(response){
        ...
        if(response.nextPageToken){
          ...
          if(component._options.mergePages) return component.mergePages(responses);
          return responses;
        }
      });
  ...
  this.mergePages = function(pages){
    if(!pages || !Array.isArray(pages) || pages.length<1 ) throw new Error('must provide an array containing pages to merge');
    var mergedPages = {
      "pageInfo":{
        "totalResults":0
      },
